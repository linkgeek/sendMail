<?php
	...
	//实例pdo对象，连接数据库
	$db = new PDO('mysql:host=localhost;dbname=imooc','root','rootroot',array(PDO::MYSQL_ATTR_INIT_COMMAND => "set names utf8"));
	while(true){
		//查询sql
		...
		if(!empty($data)){
			foreach ($data as $value) {
				//发送邮件
				$mail = sendMail('smtp.163.com','hezhan_web@163.com','web2016hezhan','941192051@163.com',$v['user_email'],'加藤非','这是主题','这是内容');
				if($mail){
					//发送后，更新status为1
